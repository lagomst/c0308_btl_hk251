\section{Task 3.1: Chặn bắt và Phân tích API SMC}
...
\begin{itemize}
    ...
    \item \textbf{Tools:} apktool, apksigner, adb
\end{itemize}
...
\subsubsection{Bypass SSL Pinning bằng APK Patching}

\textbf{Phương pháp được sử dụng:} Decompile APK, patch CertificatePinner, và rebuild.

\begin{enumerate}
    \item \textbf{Decompile APK:}
          \begin{lstlisting}[language=bash]
apktool d secure-app.apk -o secure-app-decompiled
    \end{lstlisting}

    \item \textbf{Patch CertificatePinner.smali:}
          \begin{itemize}
              \item Tìm file \texttt{CertificatePinner.smali}
              \item Sửa tất cả check methods để return immediately
              \item Bypass certificate validation logic
          \end{itemize}

    \item \textbf{Patch LoginActivity.smali:}
          \begin{itemize}
              \item Tìm \texttt{handleNetworkError} method
              \item Skip SSL error handling
              \item Allow untrusted certificates
          \end{itemize}

    \item \textbf{Rebuild và sign APK:}
          \begin{lstlisting}[language=bash]
# Rebuild APK
apktool b secure-app-decompiled -o secure-app-patched.apk

# Sign voi debug keystore
apksigner sign --ks debug.keystore secure-app-patched.apk
    \end{lstlisting}

    \item \textbf{Cài đặt patched APK:}
          \begin{lstlisting}[language=bash]
adb install -r secure-app-patched.apk
    \end{lstlisting}
\end{enumerate}

\textbf{Kết quả:} SSL pinning bypass thành công, ứng dụng có thể kết nối qua Burp proxy.

\subsubsection{Xác minh SSL Pinning Bypass}

Sau khi patch và cài đặt lại APK, thực hiện test để xác minh bypass thành công:

\begin{enumerate}
    \item \textbf{Test kết nối:}
          \begin{lstlisting}[language=bash]
# Khoi dong ung dung va kiem tra Burp Suite
# Phai thay traffic HTTP/HTTPS trong Proxy -> HTTP history
    \end{lstlisting}

    \item \textbf{Xác minh chứng chỉ:}
          \begin{itemize}
...
\begin{table}[h]
    \centering
    \small
    \begin{tabular}{|p{3.5cm}|p{3cm}|p{2cm}|p{4.5cm}|}
        \hline
...
\begin{table}[h]
    \centering
    \small
    \begin{tabular}{|p{1cm}|p{3.5cm}|p{2cm}|p{3cm}|p{3cm}|p{1.5cm}|}
        \hline
...
\begin{table}[h]
    ...
    \begin{tabular}{|p{3cm}|p{2cm}|p{8cm}|}
        ...
        \hline
        apktool        & Latest           & APK decompilation và recompilation          \\
        \hline
        apksigner      & Android SDK      & APK signing với debug keystore              \\
        \hline
...
\begin{itemize}
    ...
    \item \textbf{SSL Pinning Bypass:} APK patching technique để bypass certificate validation
...
\begin{table}[h]
    ...
    \begin{tabular}{|p{4cm}|p{4cm}|p{5cm}|}
        ...
        \hline
        SSL Certificate Pinning & Multiple validation layers & APK patching + system certificate installation \\
        \hline
