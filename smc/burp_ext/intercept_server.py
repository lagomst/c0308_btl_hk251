...
class Client:
    ...
    def verify_login_session(self, session_data:dict[str|Any], server_sig_pubkey:tuple[int,int], sig_tuple:tuple[int,int]):
        ...
        return "success" if verify_result else "error"
    ...
    def receive_msg(self, reply_iv:bytes, encrypted_reply:bytes, tags:bytes, reply_sig_pubkey:tuple[int,int], reply_rs:tuple[int,int]):
...
def main():
    client = Client()
    server = Server()
    # Get server pubkey and session
    login_session, server_pubkey, login_sesssion_ephe_pubkey, login_sesssion_rs = server.get_login_session()
    # Client verify login is legit
    status = client.verify_login_session(login_session, login_sesssion_ephe_pubkey, login_sesssion_rs)
    if status != "success":
        return
    client.server_pubkey = PointEC(CLIENT_ECDH.curve, *server_pubkey) # Save server public key
    # Client initiate key-exchange
    client_pubkey, client_rs, client_ephe_pubkey = client.key_exchange()
    # Server verify user signature
    status = server.do_key_exchange(client_pubkey, client_rs, client_ephe_pubkey)
    if status != "success":
        return
    # Both has derived their own shared secret,
    # now communicate through encrypted channel!
    # Client send msg, server receive it
    plaintext_1 = server.receive_msg(*(client.send_msg()) )
    # print("Server extracted plaintext: ", plaintext)
    # Server send msg, user receive it
    plaintext_2 = client.receive_msg(*(server.send_msg()) )
